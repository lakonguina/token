...
class FA2_core(sp.Contract):
    ...
    def mint(self, params):
        sp.verify(sp.sender == self.data.administrator, message = self.error_message.not_admin_or_operator())
...
class FA2(FA2_token_metadata, FA2_core):
    @sp.offchain_view(pure = True)
    def all_tokens(self):
        if self.config.assume_consecutive_token_ids:
    ...
    def __init__(self, config, metadata, admin):
        ...
        FA2_core.__init__(self, config, metadata, administrator = admin)

        # Add your tokens in this location
        self.update_initial_storage(
                # Amount of tokens existent
                all_tokens = 1,
                # Tokens ledger (contains owners and balances)
                ledger = sp.big_map({
                    #(<Owner>, <Token ID>): <balance>
                    sp.pair(sp.address("tz1Q7c5YR4iXS3v8CZwaUokxUqby8TADZt4U"), 0): sp.record(balance = 1000000) 
                    }),
                # Token metadata
                token_metadata = sp.big_map({
                    0: sp.record(
                        ...
                        token_info = sp.map({
                            ...
                            "name": sp.utils.bytes_of_string("DogeXTZ"),
                            "symbol": sp.utils.bytes_of_string("DOGEZ"),
                            "thumbnailUri": sp.utils.bytes_of_string("ipfs://QmccYeSvSmTJps6VBgsXrjzQBBHH4ZRDXkQ93bTbDRDvQX")
                            })
                        )
                    }),
                )
...
sp.add_compilation_target("FA2_comp", FA2(config = environment_config(),
        metadata = sp.big_map({
            ...
            "content": sp.utils.bytes_of_string("""{"version":"v0.0.1","description":"description","authors":["@goforfa"],"source":{"tools":["SmartPy"],"location":"https://gitlab.com/smondet/fa2-smartpy.git"},"interfaces":["TZIP-012","TZIP-016"],"errors":[],"views":[]}""")}),
                            admin = sp.address("tz1Q7c5YR4iXS3v8CZwaUokxUqby8TADZt4U")))
